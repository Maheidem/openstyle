...
const EXCLUSIONS = new Set(["openstyle", "electron"]);
...
test("our own windows are excluded outside practice mode", () => {
  expect(isRemixTargetAllowed("Openstyle", EXCLUSIONS, false)).toBe(false);
  expect(isRemixTargetAllowed("Electron", EXCLUSIONS, false)).toBe(false);
});

test("practice mode suspends the self-exclusion", () => {
  expect(isRemixTargetAllowed("Openstyle", EXCLUSIONS, true)).toBe(true);
