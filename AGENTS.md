...
1. Native per-platform binary (`apps/electron/native/{macos,windows,linux}-key-listener.*`, spawned by `apps/electron/src/main/key-listener.ts`) emits key-down/up → IPC to the pill (`apps/electron/src/renderer/src/pages/app.tsx`, its own `pill.html` renderer entry). (`native/` also holds the other TypeScript-spawned helpers: `{macos,windows}-mic-listener.*` for mic-in-use monitoring (`src/main/mic-listener.ts`), `macos-media-control.swift` + `{macos,windows}-output-volume.*` to pause-or-duck audio around dictation (`src/main/audio-control/`, IPC `audio:prepare`/`audio:duck`/`audio:restore`), and `macos-ax.swift` for macOS accessibility read/select in Remix document access; fast-paste, system-audio and `fluidaudio-diarize` are covered below.)
...
**Import** (merged in PR #9, on top of 2.4.0): transcribe an existing audio/video file instead of live capture, through the same pipeline. The Import screen (`apps/electron/src/renderer/src/pages/import.tsx` — drop zone, native picker via `window.api.pickImportFile`/`importAudioFile`, upload/done/error states; e2e: `apps/electron/tests/import-screen.test.ts`) hands the file path to `apps/electron/src/main/import-audio.ts`, which streams it from disk (`openAsBlob` → `FormData`) and does the multipart `POST /api/transcribe/file` (`apps/server/src/routes/transcribe-file.ts`, mounted as a sibling of `routes/transcribe.ts` — not a fork). The route bounds the upload while **streaming** — a `content-length` pre-check plus a bounded streaming multipart parser (`lib/audio/multipart-stream.ts`) that replaced `hono/body-limit` (whose chunked path itself buffered the whole body) and enforces `MAX_IMPORT_BYTES` (1 GiB) mid-body with a byte-identical 413 — plus an extension allowlist, decodes anything that isn't already a canonical 16 kHz mono PCM16 WAV via the bundled ffmpeg **file→file** (`apps/server/src/lib/audio/decode.ts`, header helpers in `lib/audio/wav.ts`), then runs `runTranscriptionPipeline()` (`apps/server/src/lib/transcription-pipeline.ts`, extracted verbatim from the batch route) — same provider/model/language resolution, vocabulary bias + leak guard, LLM cleanup and `transcription_history` row as dictation. Peak allocation ≈ one decoded WAV (the pipeline's single `readFile`), not the old 2–3× upload (`specs/import-streaming.md`, follow-up card `19fcec19` closed); the dictation lease is still held across decode + STT. Every temp artifact lives in a per-request tmp dir removed on all paths. That bundled LGPL ffmpeg (`apps/electron/scripts/download-ffmpeg.mjs`) is a hard runtime dependency of Import — the reason the `download:ffmpeg` command exists for local dev; `build:*` runs it automatically.

**Meeting import** (`specs/meeting-import.md`): `POST /api/meetings/import` (`apps/server/src/routes/meetings-import.ts`, a sibling router mounted at the same `/meetings` prefix — no `/:id` ordering hazard) turns an existing audio file into a full meeting record instead of a dictation: multipart upload (`audio` + `id` + `audio_dir`, optional `title`/`started_at`) bounded by the same `MAX_IMPORT_BYTES`/extension allowlist now shared via `apps/server/src/lib/audio/import-limits.ts` (extracted from `transcribe-file.ts`, which re-exports them), ffmpeg decode to 16 kHz mono WAV **file→file**, then a single `recorded`-status `meetings` row and a `system.wav`-only directory under `<userData>/meetings/<id>` (the WAV is placed by rename from the temp dir — the audio is never held in memory; `specs/import-streaming.md`) — **no `mic.wav`, no `sync.json`**; the pipeline already tolerates both absences. Writing to the *system* channel is deliberate: the diarizer only reads it, so imported recordings get `Them 1..N` labels the naming UI can rename. A 409 guard (existing row id, or a non-empty target dir — the load-bearing protection for a live recording's folder) fires before any decode; there is **no** dictation-activity lease (nothing STT-shaped happens server-side). The 201 response is constructed in the exact `GET /:id` shape so the renderer needs no second fetch. Renderer: drop zone on the meetings empty state plus an Import button in the master-detail rail (`apps/electron/src/renderer/src/pages/meetings.tsx`, `useMeetingImport` — click→picker and drag-drop both supported, one import in flight at a time), main IPC `meeting-import:pick-file`/`meeting-import:transcribe` in `apps/electron/src/main/meeting-import.ts` (computes `audio_dir` with the same root as `meeting-recorder.ts`, `started_at` from file mtime so back-catalog lands at its recorded date); on success it selects the new meeting and auto-fires the existing `POST /:id/transcribe` job (409 → noop). Imported meetings get the full pipeline — transcribe/diarize/speakers/summarize/enhance/retry/reveal/delete/retention — identical to recorded ones. Server tests: `apps/server/tests/meetings-import-route.test.ts`; e2e: `apps/electron/tests/meeting-import.test.ts`.
...
Gotchas:
- **Single-test filtering**: never add `--` before the path. `pnpm --filter @openstyle/server test -- tests/x.test.ts` silently drops the filter and runs all ~65 files instead of one — omit the `--`.
...
- Meeting Mode chain (read in order): `meeting-mode.md`, `meeting-diarization.md`,
  `meeting-transcription-quality.md`, `meeting-speaker-naming.md`; sibling feature:
  `meeting-import.md` (import an existing recording as a meeting).
- Other technical specs: `llm-task-profiles.md`, `dictation-language-hotkeys.md`,
  ...
  `freestyle-transcribe-ui-refactor.md`, `meeting-import.md`.
