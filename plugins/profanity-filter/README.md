...
(`GET /api/plugins/openstyle-profanity-filter/replacements`) through the
`window.openstyle` host bridge.
...
```
pnpm install
pnpm --filter @openstyle/profanity-filter build
```
