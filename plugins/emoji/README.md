...
This is a first-party Openstyle plugin. It contributes a `beforeCleanup` hook
...
2. Choose a placement mode:
   ...
   - **Replace** — swap an emotionally expressive word for a single emoji
     (e.g. `love` → ❤️, `happy` → 😊).
3. Dictate as usual. Emojis are added during cleanup.

## How it works

The plugin appends a system-prompt fragment via the `beforeCleanup` hook. The
fragment is merged into the assembled cleanup prompt, so no extra round trip is
needed. The plugin ships no model of its own and reuses your configured cleanup
settings.
