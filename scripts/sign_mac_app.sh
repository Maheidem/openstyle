#!/usr/bin/env bash
# Ad-hoc sign a local macOS Openstyle build so the MLX PyInstaller worker can run.
# Usage: ./scripts/sign_mac_app.sh [path/to/Openstyle.app]
#
...
# After signing, re-grant Accessibility for Openstyle and macos-key-listener in
# System Settings.
set -euo pipefail
...
if [[ -z "${APP}" ]]; then
  ...
  APP="${ROOT_DIR}/apps/electron/dist/mac-arm64/Openstyle.app"
fi

if [[ ! -d "${APP}" ]]; then
  echo "Openstyle.app not found: ${APP}" >&2
  exit 1
fi
